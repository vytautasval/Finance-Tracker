...
function addCardAmount() {
  const selectedCardObject = computed(() =>
    cards.list.find((card) => card.lastDigits === newCard.value)
  )

  if (selectedCardObject.value) {
    const i = cards.list.findIndex(
      (card) => card.lastDigits === selectedCardObject.value!.lastDigits
    )
...
function addCategoryAmount() {
  const selectedCategoryObject = computed(() =>
    categories.list.find((category) => category.name === newCategory.value)
  )

  if (selectedCategoryObject.value) {
    const i = categories.list.findIndex(
      (category) => category.name === selectedCategoryObject.value!.name
    )
...
const addNewOperation = () => {
  if (document.getElementById('outflow')) {
    newAmount.value = -Math.abs(newAmount.value)
  }
...
<template>
  <v-btn class="ma-2 bg-yellow-lighten-2 rounded-pill" size="x-large"
    >New Operation
          ...
          <v-row justify="center">
            <v-col cols="auto">
              <v-btn
                ...
                :disabled="!isEntryValid || !isAmountValid || !isCardNotNull"
                type="submit"
                >Submit</v-btn
              >
            </v-col>
