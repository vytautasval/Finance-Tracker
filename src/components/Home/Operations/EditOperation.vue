...
computed(() => cards.list.find(card => card.lastDigits === editingCard.value)) 
const selectedCardIndex = 
computed(() => cards.list.findIndex(card => card.lastDigits === selectedCardObject.value!.lastDigits))

const selectedCategoryObject = 
computed(() => categories.list.find(category => category.name === editingCategory.value)) 
const selectedCategoryIndex = 
computed(() => categories.list.findIndex(category => category.name === selectedCategoryObject.value!.name))
...
function adjustCategoryAmount() {    
    const initialCategoryIndex = categories.list.findIndex(category => category.name === initialCategory.value)
...
const updateOperation = () => {
    ...
    if (isEntryValid.value && isAmountValid.value) {
        editingAmount.value = Math.abs(Number((editingAmount.value))) *
        (InOutSwitch.value ? 1 : -1)
...
<template>
    <v-card class="bg-yellow-lighten-2">
        ...
        <v-table class="bg-yellow-lighten-5">
            ...
            <tbody>
                <tr
                v-for="(operationEntry, index) in operations.list" :key="index">
                    <td>{{ operationEntry.name }}                                         
                    </td>
                    <td :id="Math.sign(operationEntry.amount) === 1 ? 'green' : 'red'">{{ operationEntry.amount }}€
                    </td>
                    <td>{{ operationEntry.category }}
                    </td> 
                    <td>{{ operationEntry.date }}
                    </td> 
                    <td>{{ operationEntry.card }}                       
                    </td>
                    <td><v-btn 
                        @click="setupActions(operationEntry, index), dialog = true, selectOperation()" 
                        class="bg-white" color="black" variant="plain" icon="mdi-pencil"></v-btn>
                        <v-btn 
                        @click="setupActions(operationEntry, index), deleteOperation(index, operationEntry)" 
                        class="bg-white" color="red" variant="plain" icon="mdi-close-thick"></v-btn>
                    </td>
                </tr>
            </tbody>
        </v-table> 
                ...
                <v-card class="pa-4 bg-grey-lighten-3">
                    ...
                    <v-row justify="center">
                        <v-col cols="auto">
                            <v-switch
                            v-model="InOutSwitch"
                            inset base-color="red"  
                            color="success" 
                            :label="InOutSwitch === true ? 'Inflow' : 'Outflow'"
                            ></v-switch>
                        </v-col>
                    </v-row>
                    <v-form @keypress.enter="updateOperation(), dialog = false" @submit.prevent="updateOperation(), dialog = false">
                    <v-text-field                            
                    label="Enter an operation.."                                            
                    v-model="editingName" 
                    ></v-text-field>
                    ...
                    <v-row justify="center">
                        <v-col cols="auto">
                            <v-btn class="bg-yellow-lighten-2 rounded-pill" :disabled="!isEntryValid || !isAmountValid" type="submit">Submit</v-btn>
                        </v-col>
                    </v-row>
                </v-form>
                </v-card>
