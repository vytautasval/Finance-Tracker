...
import {filteredCards, cardSelection} from '../filtrations'
...
const selectedCardObject = computed(() => cards.list.find(card => card.lastDigits === selectedCard.value)) 
...
<template>    
        <v-card class="bg-yellow-lighten-2" title="Total Balance:">
            <v-card-subtitle>
                Total balance by card.
            </v-card-subtitle>
            <v-card-subtitle v-if="cards.list.length === 0">
                Please add a card in "Setup" first.
            </v-card-subtitle>            
            <v-card-actions class="align-start">
                <v-select :disabled="!selectedCard" label="Select card.." :items="selectableCards" v-model="selectedCard"></v-select>
                <v-card-text 
