...
import type { Operations, Categories } from '../../usedTypes'
import { operationSelection, totalAmountByCategory } from '../filtrations'
...
const totalMonthlyAmount = computed(() => operationSelection(operations, currentDate.value).value.
reduce((accumulator, operation) => accumulator + operation, 0))
      ...
      <v-card class="mt-2 bg-yellow-lighten-2">
        ...
        <v-card-subtitle>
          Expenses by month.
            ...
            <v-btn elevation=2 class="mb-4 me-2 rounded-pill bg-yellow-lighten-2" @click="goToPreviousMonth">
            Previous Month</v-btn>
            <v-btn elevation=2 class="mb-4 ms-2 rounded-pill bg-yellow-lighten-2" @click="goToNextMonth" :disabled="isNextMonthInFuture">
            Next Month</v-btn>
        ...
        <v-table class="bg-yellow-lighten-5">
          ...
          <tbody>
            <tr v-for="(category, index) in categories.list" :key="index">
              ...
              <td              
              :id="totalAmountByCategory(operations, currentDate, category.name).value === 0 ? '' : 'red'">
              {{ totalAmountByCategory(operations, currentDate, category.name).value }}€</td>            
            </tr>
          </tbody>
        </v-table>
      </v-card>
