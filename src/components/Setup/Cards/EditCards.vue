...
import type { Card, Cards, Operations } from '@/utilities/types'
...
<template>
  <v-card class="pa-4 bg-yellow-lighten-2">
    <v-card-title> Active cards: </v-card-title>
    <v-card-subtitle class="pb-2" v-if="cards.list.length > 0">
      Click the pencil to edit, or the eye to retire a card.
    </v-card-subtitle>
    <v-card-subtitle class="pb-2" v-else> No cards available. </v-card-subtitle>
    <template v-for="(card, index) in cards.list" :key="card.name">
      <v-chip class="black ma-1" variant="elevated" v-if="!card.retired">
        {{ card.lastDigits }}
        <v-btn
          class="ma-2"
    ...
    <template v-if="cards.list.some((card) => card.retired)">
      <v-card-title> Retired cards: </v-card-title>
      <v-card-subtitle class="pb-2"> Click again to reactivate. </v-card-subtitle>
      <template v-for="(card, index) in cards?.list" :key="card.lastDigits">
        <v-chip class="black ma-1" variant="elevated" v-if="card.retired">
          {{ card.lastDigits }}
          <v-btn
            class="ma-2"
