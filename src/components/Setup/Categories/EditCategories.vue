...
import type { Category, Categories, Operations} from '../../usedTypes'
...
<template>
    <v-card class="pa-4 bg-yellow-lighten-2 ">
        ...
        <template 
        v-for="(category, index) in categories.list" :key="index">
            <v-chip
            
            class="black ma-1" 
            variant="elevated"
            v-if="!category.retired">
                {{ category.name }}
                <v-btn
                class="ma-2" 
        ...
        <template 
        v-if="categories.list.some((category) => category.retired)">
            <v-card-title>
                Retired categories:
            </v-card-title>
            ...
            <template
            v-for="(category, index) in categories?.list" :key="category.name">
                <v-chip 
                class="black ma-1"                
                variant="elevated" 
                 v-if="category.retired">
            ...
            <v-card class="pa-4 bg-grey-lighten-3">
                ...
                @submit.prevent="updateCategory(), dialog = false">
                    ...
                    <v-row justify="center">
                        <v-col cols="auto">
                            <v-btn 
                            class="bg-yellow-lighten-2 rounded-pill" :disabled="!isEntryAvailable" type="submit">
                            Submit</v-btn>
                        </v-col>
                    </v-row>                    
                </v-form>
